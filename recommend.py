...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        nearest = min(centroids, key = lambda x: distance(restaurant_location(r), x))
        pairs.append([nearest, r])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    pairs = zip(list_of_x_values, list_of_y_values)
    s_xx, s_yy, s_xy = 0, 0, 0
    for x, y in pairs:
        s_xx += (x - mean(list_of_x_values)) ** 2
        s_yy += (y - mean(list_of_y_values)) ** 2
        s_xy += (x - mean(list_of_x_values)) * (y - mean(list_of_y_values))
    b = s_xy/s_xx
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor = {find_predictor(user, reviewed, f)[0]: find_predictor(user, reviewed, f)[1]
                  for f in feature_fns}
    return max(predictor, key = lambda x: predictor[x])
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            predicted_rating[name] = user_rating(user, name)
        else:
            predicted_rating[name] = predictor(restaurant)
