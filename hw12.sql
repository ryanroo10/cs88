...
CREATE TABLE long_album as
  select title from albums, tracks
		where milliseconds > 360000 and tracks.albumId = albums.albumID;
...
CREATE TABLE album_count as
  select name, count(*) from artists, albums where artists.artistId = albums.artistId
		group by albums.artistId order by name;


-- Question 8
CREATE TABLE busiest_artists as
  select artists.name as artist, count(tracks.albumId) as numberOfTracks
		from artists inner join albums on albums.artistId = artists.artistId
			inner join tracks on tracks.albumId = albums.albumId group by artists.name order by artists.name asc;
